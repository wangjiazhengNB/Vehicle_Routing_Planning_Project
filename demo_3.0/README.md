...
| 特性 | 说明 |
|------|------|
| 🔀 **多路径规划** | 利用途经点技术生成不同路径供算法选择 |
| 🧮 **多算法对比** | Dijkstra、A*、PSO 三种算法同时运行并对比结果 |
...
| 🔐 **用户认证系统** | 完整的登录/注册功能，保存登录和注册的用户信息|
| 🎨 **优化前端界面** | 天蓝色主题统一，响应式布局，结果页面优化 |

### 🎯 应用场景（设计中）

- **路线规划**：为用户提供多种路径选择，避开拥堵
- **物流配送**：为物流车选择合适配送线路，最大节约成本
- **优化分析**：采用多目标优化，综合考虑距离和成本
- **算法学习**：可视化学习最短路径算法原理，分析各种路径算法的优劣
